...
if __name__ == "__main__":
    ...
    for elem in tree.iterfind("./election/electoral-area"):
        if elem.attrib["identifier"] == "01" or  elem.attrib["identifier"] == "02":
            for item in elem.iterfind("./electoral-area/nominator/candidate"):
                ...
                if values.has_key(key) or valuekey[1] > 90:
                    print "matching at " + key 
                    item.attrib["values-liberal-conservative"] = values[valuekey[0]][4].strip()
                    item.attrib["values-right-left"] = values[valuekey[0]][5].strip()
                    item.attrib["values-green"] = values[valuekey[0]][6].strip()
